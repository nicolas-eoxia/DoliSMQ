...
/**
 * \file    view/question/ControlLine_list.php
 * \ingroup digiquali
 * \brief   List page for question group
 */
...
// Permissions
$permissiontoread   = $user->hasRight($object->module, 'control', 'read');
$permissiontoadd    = $user->hasRight($object->module, 'control', 'write');
$permissiontodelete = $user->hasRight($object->module, 'control', 'delete');
        ...
        if (!empty($toselect)) {
            foreach ($toselect as $toSelectedId) {
                $object->fetch($toSelectedId);
                    ...
                    setEventMessages($langs->trans('LockControlLine', $object->ref), []);
            }
        }
    }
